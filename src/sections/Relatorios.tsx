...
export function Relatorios({ data }: RelatoriosProps) {
  ...
  const dadosPeriodo = useMemo(() => {
    ...
    return meses.map(mesItem => {
      ...
      return {
        ...
        cmv: res.cmv,
        custosFixos: res.custosFixos,
        custosVariaveis: res.custosVariaveis,
      };
    });
    ...
    transacoes.forEach(t => {
      if (t.categoriaId) {
        ...
        if (cat) {
          ...
          categorias[cat.id].valor += t.valor;
        }
      }
    });
  ...
  const totais = useMemo(() => {
    return dadosPeriodo.reduce((acc, m) => ({
      ...
      faturamentoLiquido: acc.faturamentoLiquido + m.faturamentoLiquido,
      lucro: acc.lucro + m.lucro,
      custosFixos: acc.custosFixos + m.custosFixos,
      custosVariaveis: acc.custosVariaveis + m.custosVariaveis,
      cmv: acc.cmv + m.cmv,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className={`${totais.lucro >= 0 ? 'bg-gradient-to-br from-emerald-50 to-green-50' : 'bg-gradient-to-br from-red-50 to-rose-50'}`}>
          <CardContent className="p-4">
            <p className={`text-sm font-medium ${totais.lucro >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
              {totais.lucro >= 0 ? 'Lucro Total' : 'Prejuízo Total'}
            </p>
            <p className={`text-2xl font-bold ${totais.lucro >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
              {formatCurrency(Math.abs(totais.lucro))}
            </p>
          </CardContent>
        </Card>
